...
namespace Comments.Test.UnitTests
{
    public class CommentServiceTests : Infrastructure.TestBase
    {
        ...
        public void No_Comments_returned_when_not_logged_in()
        {
			...
			var commentService = new CommentService(new ConsultationsContext(_options, _fakeUserService, _fakeEncryption), FakeUserService.Get(isAuthenticated: false), _consultationService, _linkGenerator, _fakeHttpContextAccessor);
        ...
        public void Only_own_Comments_returned_when_logged_in()
        {
	        ...
	        var context = new ConsultationsContext(_options, userService, _fakeEncryption);
	        //var submitService = new SubmitService(context, userService, _consultationService);
			var commentService = new CommentService(new ConsultationsContext(_options, userService, _fakeEncryption), userService, _consultationService, _linkGenerator, _fakeHttpContextAccessor);
            var locationId = AddLocation(sourceURI);
        ...
        public void CommentsQuestionsAndAnswers_OnlyReturnOwnComments()
        {
	        ...
	        var context = new ConsultationsContext(_options, userService, _fakeEncryption);
	        //var submitService = new SubmitService(context, _fakeUserService, _consultationService);
			var commentService = new CommentService(context, _fakeUserService, _consultationService, _linkGenerator, _fakeHttpContextAccessor);
