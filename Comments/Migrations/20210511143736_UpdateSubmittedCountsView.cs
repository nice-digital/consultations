...
{
    public partial class UpdateSubmittedCountsView : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql($@"
				...
				SELECT	SUBSTRING(l.SourceURI, 0, CASE WHEN (PATINDEX('%document%', l.SourceURI) = 0) THEN LEN(l.SourceURI) + 1 ELSE PATINDEX('%document%', l.SourceURI) - 1 END) AS SourceURI, 
						...
						s.RespondingAsOrganisation,
						COALESCE(c.StatusID, a.StatusID) As StatusId
				FROM [Location] l

				LEFT JOIN Comment c ON l.LocationID = c.LocationID AND (c.StatusID = 3 OR c.StatusID = 2)
				...
				LEFT JOIN Answer a ON q.QuestionID = a.QuestionID AND (a.StatusID = 3 OR a.StatusID = 2)
				...
				COALESCE(c.OrganisationID, a.OrganisationID), s.RespondingAsOrganisation, COALESCE(c.StatusID, a.StatusID)
			");
        }
