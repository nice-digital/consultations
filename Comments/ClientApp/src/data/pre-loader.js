...
export const preload = (staticContext, endpoint) => {

	let data = null;

	// Client - get data from global var
	if (typeof (window) !== "undefined") {
		console.info(`Found preloaded data on the client with an endpoint key of ${endpoint}`);
	...
	if (staticContext.preload.data[endpoint]) {
		console.log(`Data with key '${endpoint}' has already been preloaded on the server`);
