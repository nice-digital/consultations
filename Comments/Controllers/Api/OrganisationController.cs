	...
	public class OrganisationController : ControllerBase
    {
		...
		public async Task<IActionResult> CheckOrganisationUserSession(int consultationId, Session session)
		{
			...
			if (session.SessionCookies.ContainsKey(consultationId))
				throw new ArgumentException("Session id is not valid", nameof(session));


			var validityAndOrganisationName = await _organisationService.CheckOrganisationUserSession(consultationId, session);
